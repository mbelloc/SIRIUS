...
Image Interpolator::Interpolate2D(const Image& image,
    ...
    float beta = parameters.row_shift - static_cast<int>(parameters.row_shift);
    float alpha = parameters.col_shift - static_cast<int>(parameters.col_shift);
    ...
    return RemoveBorders(interpolated_im,
                         static_cast<int>(std::ceil(-parameters.row_shift)),
                         static_cast<int>(std::ceil(-parameters.col_shift)));
}
