...
PeriodicSmoothPolicy::PeriodicSmoothPolicy(
      IImageProcessor::UPtr image_processor, IInterpolator2D::UPtr interpolator)
    : image_processor_(std::move(image_processor)),
      interpolator_(std::move(interpolator)) {}

Image PeriodicSmoothPolicy::DecomposeAndProcess(const Image& image) const {
    ...
    auto zoomed_image = image_processor_->Process(periodic_part_image);
    ...
    auto interpolated_smooth_image =
          interpolator_->Interpolate2D(smooth_part_image);
